...
namespace DotNetCoreKoans.Koans
{
    public class AboutDelegates : Koan
    {
        ...
        {
            //Like strings it looks like you can change what a delegate references,
            // but really they are immutable objects
            MyMath m = new MyMath();
            BinaryOp a = m.Add;
            BinaryOp original = a;
            Assert.Same(a, original);
            // a = MyMath.Subtract;
            //a is now a different

            // Oh yeah, they are immutable, so this wouldn't work
            // a = m.Add;
            Assert.Same(a, original);
        }
        ...
        public void OnlyLastResultReturned()
        {
            ...
            Curry adding = f.Add5;
            adding += f.Add10;
            // Delegates may have more than one method attached,
            // but only the result of the last method is returned.
